...
for key in c2:
    ...
    if conv_match:
        ...
        new_key = 'layer%d.%d.%s%d.%s' % (layer - 1, block, name, module,
                                          suffix)
        key_map[new_key] = key
...
new_state_dict = {
    key: torch.from_numpy(c2[key_map[key]])
    for key in state_dict if key in key_map
}
...
for key in state_dict:
    ...
    print(f'{key_map[key]:23s} --> {key:35s} | {str(tuple(c2_v.shape)):21s}')
