...
def roi_l(mode):
    ...
    """
    data_path = "data/roi_features.pt"
    ...
    data_info = pd.DataFrame(data_info, columns=["c_num", "p_num"])
    # print(data_info)
    data_info_new = data_info.loc[data_info.index.repeat(
        data_info.c_num)]
    data_info_new = data_info_new.reset_index(drop=True)
    # print(data_info_new)
    ...
    # print(labels)
    labels_new = labels.loc[labels.index.repeat(data_info_new.p_num)]
...
def r3d_l(mode):
    ...
    """
    data_path = "data/r3d_features.pt"
